  ...
  return (
    ...
    >
      ...
      <div className="card-content">
        ...
        <span className="card-value">{param.value}</span>
        {param.description && (
          <span className="card-description">{param.description}</span>
        )}
      </div>
    </motion.div>
  );
...
function ExpandedCard({ param, setExpanded }) {
  ...
  const getChartOptions = () => {
    ...
    if (isCombinedChart) {
      ...
      return {
        ...
        xaxis: {
          categories: param.series[0]?.data?.map((_, index) => `Période ${index + 1}`) || [],
          labels: { 
            style: { 
              colors: '#fff',
              fontSize: '12px',
  ...
  return (
    ...
    >
      ...
      <div className="details-section">
        <div className="details-header">
          <h4>Détails par région</h4>
          <span className="items-count">{param.details?.length || 0} éléments</span>
        </div>
        <div className="details-grid">
          {param.details && param.details.slice(0, 8).map((item, index) => (
            <div key={index} className="detail-item">
              <span className="detail-label">
                {item.Region && `${item.Region} - `}
                {item.Titre || item.Domaine || item.label || "Non spécifié"}
              </span>
              <span className="detail-value">{item.count}</span>
            </div>
          ))}
          {param.details && param.details.length > 8 && (
            <div className="more-items">
              + {param.details.length - 8} autres éléments
            </div>
          )}
        </div>
      </div>
