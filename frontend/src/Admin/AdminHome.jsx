...
const AdminHome = () => {
  const [adminStats, setAdminStats] = useState({ totalDentistes: 0, pending: 0, verified: 0, regions: 0 });
  const [profilsRecents, setProfilsRecents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  ...
  const CHART_COLORS = ['#87CEEB', '#5DADE2', '#3498DB', '#2980B9', '#21618C', '#1B4F72'];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [
          regions, titres, domaines, total, titreReg, domaineReg,
          adminStatsRes, recent
        ] = await Promise.all([
          axios.get(`${API_BASE}/stats-region`).then(r => r.data.data || []),
          axios.get(`${API_BASE}/stats-titre`).then(r => r.data.data || []),
          axios.get(`${API_BASE}/stats-domaine`).then(r => r.data.data || []),
          axios.get(`${API_BASE}/totals`).then(r => r.data.data || { total: 0 }),
          axios.get(`${API_BASE}/stats-titre-region`).then(r => r.data.data || []),
          axios.get(`${API_BASE}/stats-domaine-region`).then(r => r.data.data || []),
          axios.get(`${ADMIN_API_BASE}/stats`).then(r => r.data || {}),
          axios.get(`${ADMIN_API_BASE}/recent`).then(r => r.data || [])
        ]);

        setRegionData(regions.map(r => ({ name: r.Region || "Inconnu", dentistes: +r.count || 0 })).sort((a, b) => b.dentistes - a.dentistes));
        setTitreData(titres.map(t => ({ name: t.Titre || "Inconnu", value: +t.count || 0 })).sort((a, b) => b.value - a.value));
        setDomaineData(domaines.map(d => ({ name: d.Domaine || "Inconnu", value: +d.count || 0 })).sort((a, b) => b.value - a.value));
        setTitreRegionData(titreReg);
        setDomaineRegionData(domaineReg);
        setProfilsRecents(recent);

        setAdminStats({
          totalDentistes: total.total || 0,
          pending: adminStatsRes.pending || 0,
          verified: adminStatsRes.verified || 0,
  ...
  const getNomComplet = p => ` ${p.Nom || ""} ${p.Prenom || ""}`.trim() || "Inconnu";
  const getInitiales = p => getNomComplet(p).split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 3) || "??";
  const getRegion = p => p.Region || "Non spécifié";
  ...
  const SmallCharts = () => (
    <div className="small-charts-grid">
      ...
      <div className="small-chart-card">
        <h4>Répartition par Titre</h4>
        <ResponsiveContainer width="100%" height={230}>
          <PieChart>
            <Pie data={titreData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={85}
              label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
              {titreData.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
            </Pie>
            <Tooltip />
          </PieChart>
        </ResponsiveContainer>
      ...
      <div className="small-chart-card">
        ...
        <ResponsiveContainer width="100%" height={230}>
          <BarChart data={domaineData}>
            ...
            <YAxis />
            <Tooltip />
            <Bar dataKey="value" fill="#5DADE2" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div className="small-chart-card">
        <h4>Dentistes par Région</h4>
        <ResponsiveContainer width="100%" height={230}>
          <BarChart data={regionData.slice(0, 8)}>
            ...
            <YAxis />
            <Tooltip />
            <Bar dataKey="dentistes" fill="#87CEEB" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
  ...
  // === Gros graphique : Titres par région ===
  const ChartTitreRegion = () => {
    const data = Object.values(titreRegionData.reduce((acc, cur) => {
      const r = cur.Region || "Inconnu";
      if (!acc[r]) acc[r] = { region: r };
      acc[r][cur.Titre || "Inconnu"] = +cur.count || 0;
      return acc;
    }, {}));

    const titres = [...new Set(titreRegionData.map(x => x.Titre || "Inconnu"))];
    ...
    return (
      <ResponsiveContainer width="100%" height={420}>
        <LineChart data={data}>
          ...
          <Legend />
          {titres.map((t, i) => (
            <Line key={t} type="monotone" dataKey={t} stroke={CHART_COLORS[i % CHART_COLORS.length]} strokeWidth={6} dot={{ r: 8 }} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    );
  ...
  // === Gros graphique : Domaines par région ===
  const ChartDomaineRegion = () => {
    const data = Object.values(domaineRegionData.reduce((acc, cur) => {
      const r = cur.Region || "Inconnu";
      if (!acc[r]) acc[r] = { region: r };
      acc[r][cur.Domaine || "Inconnu"] = +cur.count || 0;
      return acc;
    }, {})).slice(0, 10);

    const domaines = [...new Set(domaineRegionData.map(x => x.Domaine || "Inconnu"))];
    ...
    return (
      <ResponsiveContainer width="100%" height={420}>
        <BarChart data={data}>
          ...
          <Legend />
          {domaines.map((d, i) => (
            <Bar key={d} dataKey={d} fill={CHART_COLORS[i % CHART_COLORS.length]} radius={[8, 8, 0, 0]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    );
  ...
  return (
    <div className="admin-accueil">
      ...
      <header className="admin-header">
        ...
        <div className="admin-actions">
          <Link to="admin/valide" className="btn primary">
            Valider les profils ({adminStats.pending || 0})
          </Link>
        </div>
      </header>
      ...
      <div className="quick-stats-grid">
        <div className="stat-card">
          ...
          <div className="number">{adminStats.totalDentistes}</div>
        </div>
        <div className="stat-card">
          <h3>En attente</h3>
          <div className="number">{adminStats.pending}</div>
        </div>
        <div className="stat-card">
          <h3>Régions</h3>
          <div className="number">{adminStats.regions}/22</div>
        </div>
        <div className="stat-card">
          <h3>Vérifiés</h3>
          <div className="number">{adminStats.verified}</div>
        </div>
      </div>

      {/* Profils récents + 3 petits graphiques */}
      <div className="top-section">
        <div className="profils-recents">
          <h2>Derniers profils ajoutés</h2>
          <div className="profils-list">
            {profilsRecents.length > 0 ? profilsRecents.map((p, i) => (
              <div key={i} className="profil-item">
                <div className="profil-avatar">{getInitiales(p)}</div>
                <div className="profil-info">
                  <h4>{getNomComplet(p)}</h4>
                  <p>{getRegion(p)}</p>
                </div>
              </div>
      ...
      {/* LES 2 GROS GRAPHIQUES — BIEN GRANDS */}
      <div className="big-charts-section">
        <div className="big-chart-wrapper">
          <h3>Répartition des Titres par Région</h3>
          <ChartTitreRegion />
        </div>
        <div className="big-chart-wrapper">
          <h3>Répartition des Domaines par Région</h3>
          <ChartDomaineRegion />
        </div>
      </div>
